...
if (!($isDev || $isCreator || $isCollaborator)) { http_response_code(403); die("Permissão negada."); }
...
if (!empty($event['palestrantes_ids'])) {
    ...
    if (json_last_error() === JSON_ERROR_NONE && is_array($tmp)) $current_palestrantes = array_map('intval', $tmp);
}
?>
<!DOCTYPE html>
<html lang="pt-BR">
<head>
    ...
    <title>Adicionar Palestrantes</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet">
    <style>
        /* (Estilos Idênticos ao Collaborators.php) */
        ...
        .btn-add { background: #007bff; color: #fff; width: 100%; margin-top: 15px; font-size: 16px; }
...
<div class="main-content">
    <div class="card">
        <div class="top">
            <h2>Palestrantes — <?= htmlspecialchars($event['nome']) ?></h2>
            <button class="btn btn-back" onclick="location.href='index.php'">Voltar</button>
        </div>

        <p>Marque quem irá palestrar neste evento:</p>
        ...
        <button id="btnAdd" class="btn btn-add">Salvar Palestrantes</button>
    </div>
</div>
...
<script>
    ...
    document.getElementById('btnAdd').addEventListener('click', async () => {
        ...
        try {
            ...
            let json; try { json = text ? JSON.parse(text) : {}; } catch (e) { alert('Resposta inválida do servidor'); return; }

            if (!res.ok) { alert(json.erro || 'Erro ao salvar.'); return; }
            alert('Palestrantes atualizados!');
