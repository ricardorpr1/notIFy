...
$eventos = [];
try {
    // 1. Tentar a query moderna (com JSON_CONTAINS)
    ...
    // 2. Se a query moderna falhar, usar o FALLBACK com LIKE
         ...
         try {
            $sql_fallback = "SELECT id, nome, data_hora_inicio, data_hora_fim, local, created_by, colaboradores_ids, palestrantes_ids, inscricoes, presencas
                             ...
                             WHERE 
                                 created_by = :user_id 
                                 OR (
                                    colaboradores_ids LIKE :id_exact 
                                    OR colaboradores_ids LIKE :id_start
                                    OR colaboradores_ids LIKE :id_middle
                                    OR colaboradores_ids LIKE :id_end
                                 )
                                 OR (
                                    palestrantes_ids LIKE :id_exact 
                                    OR palestrantes_ids LIKE :id_start
                                    OR palestrantes_ids LIKE :id_middle
                                    OR palestrantes_ids LIKE :id_end
                                 )
                                 OR (
                                    inscricoes LIKE :id_exact 
                                    OR inscricoes LIKE :id_start
                                    OR inscricoes LIKE :id_middle
                                    OR inscricoes LIKE :id_end
                                 )
                             ORDER BY data_hora_inicio DESC";
            ...
            $eventos = $stmt_fallback->fetchAll();
             ...
             die("Erro ao buscar eventos (fallback): " . $e2->getMessage());
        }
    } else {
...
<head>
    ...
    <style>
        ...
        .event-actions .btn-cert { background: #17a2b8; color: #fff; padding: 8px 12px; border: none; border-radius: 6px; cursor: pointer; text-decoration: none; font-weight: bold; display: inline-block; font-size: 14px; }
        .event-actions .btn-cert:hover { background: #138496; }
        .event-actions .btn-cert:disabled { background: #ccc; cursor: not-allowed; opacity: 0.7; }
        ...
        <ul class="event-list">
            ...
            <?php else: ?>
                <?php foreach ($eventos as $evento): ?>
                    ...
                    // Lógica de Papel (sem alterações de prioridade, apenas lista)
                    $papeis = [];
                    ...
                    <li class="event-item">
                        ...
                        <div class="event-actions">
                            ...
                            if ($evento_terminou && $esteve_presente) {
                                echo '<a href="gerar_certificado.php?event_id=' . $evento['id'] . '" class="btn-cert" target="_blank">Baixar Certificado</a>';
                            } elseif ($evento_terminou && !$esteve_presente) {
                                echo '<button class="btn-cert" disabled title="Presença não registrada neste evento.">Baixar Certificado</button>';
                            }
                            ?>
                        </div>
                    </li>
