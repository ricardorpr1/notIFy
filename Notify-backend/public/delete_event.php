...
// Upload config e FunÃ§Ãµes Helper (sem alteraÃ§Ãµes)
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') { respond(405, ["erro" => "MÃ©todo nÃ£o permitido. Use POST."]); }
if (!isset($_SESSION['usuario_id'])) { respond(401, ["erro" => "UsuÃ¡rio nÃ£o autenticado."]); }
$me = intval($_SESSION['usuario_id']);
$myRole = intval($_SESSION['role'] ?? 0);

$data = $_POST;
$eventoId = isset($data['id']) ? intval($data['id']) : 0;
if ($eventoId <= 0) respond(400, ["erro" => "ID do evento invÃ¡lido."]);

try {
    $pdo = new PDO("mysql:host={$DB_HOST};port={$DB_PORT};dbname={$DB_NAME};charset=utf8mb4", $DB_USER, $DB_PASS, [
        PDO::ATTR_ERRMODE => PDO::ERRMODE_EXCEPTION, PDO::ATTR_EMULATE_PREPARES => false
    ]);
} catch (PDOException $e) { respond(500, ["erro" => "Erro de conexÃ£o com o banco."]); }

try {
    $stmt = $pdo->prepare("SELECT * FROM eventos WHERE id = :id LIMIT 1");
    $stmt->execute([':id' => $eventoId]);
    $event = $stmt->fetch(PDO::FETCH_ASSOC);
    if (!$event) respond(404, ["erro" => "Evento nÃ£o encontrado."]);
...
if (!($isDev || ($createdBy !== null && $createdBy === $me) || $isCollaborator)) {
    respond(403, ["erro" => "PermissÃ£o negada."]);
}

// (Processamento de Uploads - sem alteraÃ§Ãµes)
$capa_url_db = null;
...
try {
    if (isset($_FILES['capa_upload']) && $_FILES['capa_upload']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!array_key_exists($fMime, $ALLOWED_MIMES)) throw new Exception('Tipo de arquivo de capa nÃ£o permitido.');
    ...
    if (isset($_FILES['imagem_completa_upload']) && $_FILES['imagem_completa_upload']['error'] === UPLOAD_ERR_OK) {
        ...
        if (!array_key_exists($fMime, $ALLOWED_MIMES)) throw new Exception('Tipo de arquivo de imagem completa nÃ£o permitido.');
...
$updatableText = [
    ...
    'limite_participantes', 'colaboradores', // 'colaboradores' (nomes) Ã© legado
    'colaboradores_ids', 'palestrantes_ids'
    // 'turmas_permitidas' Ã© tratado separadamente
];
...
foreach ($updatableText as $key) {
    ...
    if ($key === 'data_hora_inicio' || $key === 'data_hora_fim') {
        ...
        if ($val === null && !empty($data[$key])) respond(400, ["erro" => "Formato de data invÃ¡lido para $key."]);
...
if (isset($data['turmas_permitidas'])) {
    ...
    // Adiciona "0" se o pÃºblico externo foi marcado
